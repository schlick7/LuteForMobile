                ...
                "installLute3" -> {
                    ...
                    scope.launch {
                        ...
                        val installResult = installLute3Chained(context) { stepName, stepStatus, maxWaitSeconds ->
                            android.util.Log.d(
                                "TermuxBridge",
                                "Chained installation progress: $stepName - $stepStatus, sink: ${eventSink != null}"
                            )
                            try {
...
private suspend fun checkExternalAppsByFileCreation(context: Context): Boolean {
    ...
    val testDir =
        "${android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_DOWNLOADS)}/termux_external_test_dir"
